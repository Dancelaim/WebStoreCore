...
namespace Admin.Controllers
{
    ...
    public class ProductOptionParamController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetOptionParams(ProductOptionParamRequest request)
        {
            ...
            var OptionParams = await _context.ProductOptionParams.Skip(request.Skip).Take(request.Quantity).Select(p => new ProductOptionParam { ParameterId = p.Id, ParameterName = p.Name }).ToListAsync();
        ...
        public async Task<IActionResult> GetSearchMethodForOptionParams(string Name, int Quantity)
        {
            ...
            var OptionParams = await _context.ProductOptionParams.Take(Quantity).Where(c => c.Name.StartsWith(Name) || c.Name.Contains(Name) || c.Name.EndsWith(Name)).Select(p => new ProductOptionParam { ParameterId = p.Id, ParameterName = p.Name }).ToListAsync();
            if (OptionParams.Count == 0)
            {
                result.Code = -100;
                result.Message = "Can't get products with given parameters.";
        ...
        public async Task<IActionResult> GetOptionParam(Guid ParamId)
        {
            ...
            var OptionParam = await _context.ProductOptionParams.Where(p => p.Id == ParamId).Select(p => new ProductOptionParam { ParameterName = p.Name, ParameterPrice = p.ParameterPrice, ParameterSale = p.ParameterSale, ParameterTooltip = p.ParameterTooltip, ParentOptionId = p.ParentOptionId, ParameterParentId = p.ParameterParentId }).FirstOrDefaultAsync();
            if (OptionParam == null)
            {
                response.Code = -100;
