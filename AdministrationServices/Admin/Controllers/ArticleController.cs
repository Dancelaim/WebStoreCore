...
namespace Admin.Controllers
{
    ...
    public class ArticleController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetSearchMethodForArticles(string Name, int Quantity)
        {
            var result = new ArticleResponse();

            var article = await _context.Article.Take(Quantity).Where(c => c.Title.StartsWith(Name) || c.Title.Contains(Name) || c.Title.EndsWith(Name)).Select(p => new Article { ArticleId = p.ArticleId, Title = p.Title }).ToListAsync();
            if (article.Count == 0)
            {
                result.Code = -100;
                result.Message = "Can't get products with given parameters.";
