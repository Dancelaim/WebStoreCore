...
public class AdminController : Controller
{
    ...
    //[Authorize(Roles = "Root Admin,Admin,Price Admin")]
            ...
            //            if (prod != null)
            //            {
            //                ViewData["GamesList"] = new SelectList(_context.ProductGames.Select(g => g.GameName), prod?.ProductGame.GameName ?? "Select Game");
            ...
            //                {
            //                    ViewData["GamesList"] = new SelectList(_context.ProductGames.Select(g => g.GameName), prod?.ProductGame.GameName ?? "Select Game");
            //                    ViewData["CategoriesList"] = new SelectList(_context.ProductGames.Where(g => game == null || g.GameName == game).SelectMany(g => g.ProductCategories).Select(p => p.ProductCategoryName), "Select Category");
            //                    ViewData["MetaTagTitleList"] = new SelectList(_context.Seos.Select(s => s.MetaTagTitle), "Select Meta tag title from List");
            //                };
            //            }
